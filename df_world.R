...
df_world_stats <- data.frame (
        cases = format(sum(df_world_countries()$total_cases), decimal.mark = ",", big.mark=".", digits = 7, nsmall = 0),
        deaths = format(sum(df_world_countries()$total_deaths), decimal.mark = ",", big.mark=".", digits = 2, nsmall = 0),
        recovered = NA
)
df_world_stats
